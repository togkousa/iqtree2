...
Modified IQ-TREE v2.3.6
=======

This is a slightly modified version of [IQ-TREE v2.3.6](https://iqtree.github.io/), originally developed by Bui Quang Minh and collaborators.
...
The code is redistributed under the original GNU General Public License. See LICENSE for details.

## How it works
The software runs the exact same algorithm. Additionally, users have the option to store all intemediate topologies visited by IQ-TREE, which **only improve** the log-likelihood score. Users can store all the improving topologies by using the `-wt` option, as in standard IQ-TREE. In standard IQ-TREE, however, this command stores not only the improving topologies, but instead all possible NNI topologies which are being evaluated throughout the tree search. The exact strategy to sample only the improving topologies is described in fully detailed in our recent study (CITE). Example usage using a single random starting tree generated by RAxML-NG:

```
./build/iqtree2 -s data/test.phy -m GTR+G -t data/random_starting.Tree --seqtype DNA --seed 0 --epsilon 0.1 -wt
```

All the improving topologies are stored in `data/test.phy.treels` file.
