...
This is a slightly hacked version of IQ-TREE 2. The software runs the exact same algorithm. Additionally, users have the option to store all intemediate topologies visited by IQ-TREE, which **only improve** the log-likelihood score. Users can store all the improving topologies by using the `-wt` option, as in standard IQ-TREE. In standard IQ-TREE, however, this command stores not only the improving topologies, but instead all possible NNI topologies which are being evaluated throughout the tree search. The exact strategy to sample only the improving topologies is described in fully detailed in our recent prerpint (CITE). Example usage using a single random starting tree generated by RAxML-NG:
